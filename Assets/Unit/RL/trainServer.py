...
class RLConfig:
    OBS_FORMAT = "<i4fifi"  # unit_id, dx, dy, selfHp, targetHp, alive, reward, done
    OBS_SIZE = struct.calcsize(OBS_FORMAT)

    ACTION_FORMAT = "<ii"  # unit_id, action_index
    ACTION_SIZE = struct.calcsize(ACTION_FORMAT)

    OBS_SHAPE = (4,)   # dx, dy, selfHp, targetHp
    NUM_ACTIONS = 5    # None, Move, Stop, Attack, Action
    ...
    def unpack_observation(data_bytes, offset):
        ...
        unit_id, dx, dy, self_hp, target_hp, alive, reward, done = raw
        return {
            "unit_id": unit_id,
            "obs_vector": [dx, dy, self_hp, target_hp],
            "alive": bool(alive),
            "reward": reward,
            "done": bool(done)
        }
