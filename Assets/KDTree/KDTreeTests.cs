...
public class KDTreeTests : MonoBehaviour
{
    ...
    struct KdSearchJob : IJobParallelFor
    {
        ...
        {
            answersJob[i] = kdJob.FindNearest(queriesJob[i], 0);
        }
    }
    ...
    void Start()
    {
        ...
        teamNative = new NativeArray<byte>(nData, Allocator.Persistent);

        for (int i = 0; i < pointsBuildIn.Length; i++)
        {
            pointsNative[i] = pointsBuildIn[i];
        }
        for (int i = 0; i < teamNative.Length; i++)
        {
            teamNative[i] = 0;
        }
    ...
    float NeighbourDistancesSumNative()
    {
        ...
        kdTreeNative.MakeFromPoints(pointsNative, teamNative);

        float neighbourDistancesSum = 0f;

        for (int i = 0; i < queries.Length; i++)
        {
            int id = kdTreeNative.FindNearest(queries[i], 0);
            neighbourDistancesSum = neighbourDistancesSum + math.length(queries[i] - pointsNative[id]);
        }
    ...
    void Update()
    {
        ...
        else if (imode == 1)
        {
            for (int i = 0; i < queries.Length; i++)
            {
                int j = kdTreeNative.FindNearest(queries[i], 0);
